...
<%   
        ...
        String hospital_opassword = request.getParameter("hospital_opassword");
        String hospital_npassword = request.getParameter("hospital_npassword");
		try{
			   ...
			   ResultSet rs=st.executeQuery("SELECT hospital_username,hospital_password FROM hospital WHERE hospital_username='"+hospital_username+"' AND hospital_password = '"+hospital_opassword+"'");
			   try{ 
                   rs.next();
		
		           if(rs.getString("hospital_username").equals(hospital_username) && rs.getString("hospital_password").equals(hospital_opassword)){
					response.setContentType("text/html");            
						try{  

									PreparedStatement ps=con.prepareStatement("UPDATE hospital SET hospital_password = '"+hospital_npassword+"' WHERE hospital_username='"+hospital_username+"'");    
                                    %> 
                                    <script>
                                          var m = "Password changed!! redirecting to Login page";
                                          alert(m);
